...
- Multipass (tested on Multipass 1.16.1)
  - Or other VMs if you prefer to run only the Ansible playbooks (manual VM setup required)
...
- If you don't have an existing `~/.ssh/config`, copy the sample file:
...
- If you already have an SSH config file, append the sample contents instead of overwriting it.
- This makes it easier to SSH into the Kubernetes nodes without specifying the private key and host manually.
...
# Adjust the number of master and worker nodes based on available resources
```
